...
describe('PartnersService', () => {
  ...
  describe('findAll', () => {
    it('should return paginated partners', async () => {
      const partners = [
        { id: 'partner-1', companyId: 'company-1', name: 'Client A', type: PartnerType.CLIENT },
      ];
    ...
    it('should apply type filter when provided', async () => {
      ...
      await service.findAll('company-1', { page: 1, limit: 20, type: PartnerType.SUPPLIER });
    ...
    it('should apply search filter when provided', async () => {
      ...
      await service.findAll('company-1', { page: 1, limit: 20, search: 'Acme' });
  ...
  describe('findOne', () => {
    it('should return a partner by id', async () => {
      const partner = { id: 'partner-1', companyId: 'company-1', name: 'Client A' };
      ...
      expect(mockPrismaService.partner.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 'partner-1', companyId: 'company-1', deletedAt: null } }),
      );
    ...
    it('should throw NotFoundException if partner not found', async () => {
      ...
      await expect(service.findOne('partner-999', 'company-1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('create', () => {
    it('should create and return a partner', async () => {
      const dto = { name: 'New Partner', type: PartnerType.CLIENT, personType: 'PJ' as any };
      ...
      expect(mockPrismaService.partner.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ companyId: 'company-1', name: 'New Partner', type: PartnerType.CLIENT }),
        }),
      );
  ...
  describe('update', () => {
    it('should call findOne then update and return the partner', async () => {
      const existing = { id: 'partner-1', companyId: 'company-1', name: 'Client A' };
      ...
      const result = await service.update('partner-1', 'company-1', { name: 'Client A Updated' } as any);
    ...
    it('should throw NotFoundException if partner does not exist', async () => {
      ...
      await expect(service.update('partner-999', 'company-1', {} as any)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('remove', () => {
    it('should soft delete by setting deletedAt', async () => {
      const existing = { id: 'partner-1', companyId: 'company-1', name: 'Client A' };
      mockPrismaService.partner.findFirst.mockResolvedValue(existing);
      mockPrismaService.partner.update.mockResolvedValue({ ...existing, deletedAt: new Date() });
    ...
    it('should throw NotFoundException if partner does not exist', async () => {
      ...
      await expect(service.remove('partner-999', 'company-1')).rejects.toThrow(NotFoundException);
    });
  });
});
