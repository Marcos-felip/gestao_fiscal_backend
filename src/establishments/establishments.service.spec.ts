...
describe('EstablishmentsService', () => {
  ...
  describe('create', () => {
    it('should create and return a FILIAL establishment', async () => {
      ...
      expect(mockPrismaService.establishment.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ companyId: 'company-1', name: 'Filial SP' }) }),
      );
    ...
    it('should create a MATRIZ establishment when none exists', async () => {
      ...
      expect(mockPrismaService.establishment.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ companyId: 'company-1', type: EstablishmentType.MATRIZ }),
        }),
      );
      expect(result).toEqual(created);
    });
    ...
    it('should throw ConflictException if MATRIZ already exists', async () => {
      ...
      mockPrismaService.establishment.findFirst.mockResolvedValue({ id: 'existing' });

      await expect(service.create('company-1', dto as any)).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('findAll', () => {
    it('should return all establishments for a company', async () => {
      const establishments = [
        { id: 'est-1', companyId: 'company-1', name: 'Matriz', type: EstablishmentType.MATRIZ },
        { id: 'est-2', companyId: 'company-1', name: 'Filial', type: EstablishmentType.FILIAL },
      ];
  ...
  describe('findOne', () => {
    it('should return an establishment by id', async () => {
      const establishment = { id: 'est-1', companyId: 'company-1', name: 'Matriz' };
      mockPrismaService.establishment.findFirst.mockResolvedValue(establishment);
    ...
    it('should throw NotFoundException if establishment not found', async () => {
      ...
      await expect(service.findOne('est-999', 'company-1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('update', () => {
    it('should update and return the establishment', async () => {
      const establishment = { id: 'est-1', companyId: 'company-1', name: 'Matriz', type: EstablishmentType.MATRIZ };
      const updated = { ...establishment, name: 'Matriz Updated' };
      mockPrismaService.establishment.findFirst.mockResolvedValue(establishment);
      mockPrismaService.establishment.update.mockResolvedValue(updated);

      const result = await service.update('est-1', 'company-1', { name: 'Matriz Updated' } as any);
  ...
  describe('remove', () => {
    it('should soft delete a FILIAL establishment', async () => {
      const establishment = { id: 'est-2', companyId: 'company-1', name: 'Filial', type: EstablishmentType.FILIAL };
    ...
    it('should throw BadRequestException when trying to remove MATRIZ', async () => {
      ...
      await expect(service.remove('est-1', 'company-1')).rejects.toThrow(BadRequestException);
    });
  });
});
