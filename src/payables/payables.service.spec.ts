...
describe('PayablesService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a single PAGAR entry without the numbering suffix', async () => {
      mockPrismaService.financialEntry.create.mockImplementation((args) =>
        Promise.resolve({ ...entry(), ...args.data }),
      );
  ...
  describe('findAll', () => {
    it('should always scope to PAGAR within the active company', async () => {
    ...
    it('should map supplierId to partnerId and keep purchaseId', async () => {
      mockPrismaService.financialEntry.findMany.mockResolvedValue([]);
      mockPrismaService.financialEntry.count.mockResolvedValue(0);
