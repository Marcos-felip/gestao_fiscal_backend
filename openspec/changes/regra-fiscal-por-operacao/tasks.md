...
>
> **A costura chegou a ser implementada e foi revertida.** A porta
> `IRegraFiscal`, a `CadastroDoProdutoRule`, `montarItens` perguntando à porta e
> a `regraAplicada` no snapshot existiram nos commits `b238e3c` e `164a6f5`, e
> saíram no revert seguinte.
>
> Motivo: a única segunda implementação prevista no roteiro era a devolução
> (etapa 5), que foi adiada junto. Sem ela, a porta ficou sem nenhum caso de
> operação variável à vista — indireção pura, com `regraAplicada` gravando o
> mesmo valor em todo documento fiscal, para sempre, no snapshot congelado.
>
> **O desenho continua registrado no `design.md`.** Refazer a costura é meio dia
> de trabalho, e refazê-la com o segundo caso em mãos é melhor do que tê-la
> adivinhado antes.
>
...
>
> **O que reabre:** venda para fora do estado, a devolução (também adiada), ou
> produto cuja resposta dependa do comprador dentro da mesma UF. Ver
> `ROADMAP_FISCAL.md`.
